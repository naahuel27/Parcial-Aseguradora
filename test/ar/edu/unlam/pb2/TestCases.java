...
public class TestCases{
	...
	public void queSePuedaEmitirUnaPolizaDeVida() {
		final Integer NUMERO_DE_POLIZA = 1, CANTIDAD_DE_BENEFICIARIOS = 2;
		final Double SUMA_ASEGURADA = 2000000.0, PRIMA = 5000.0, PREMIO = 6000.0;
		
		Persona asegurado = new Persona("Camila", 45987345, 24);
		Persona hijo = new Persona("Arturo", 65456231, 2);
		Persona conyuge = new Persona("Arturo", 65456233, 2);
		...
		assertEquals(CANTIDAD_DE_BENEFICIARIOS, poliza.obtenerCantidadDeBeneficiarios());
		assertEquals(asegurado, ((Poliza) poliza).getAsegurado());
		assertEquals(PREMIO, ((Poliza) poliza).getPremio());
	}
	...
	public void queSePuedaEmitirUnaPolizaDeCombinadoFamiliar() {
		final Integer NUMERO_DE_POLIZA = 1, CANTIDAD_DE_BENEFICIARIOS = 3;
		...
		Persona asegurado = new Persona("Camila", 45987345, 24);
		Persona hijo = new Persona("Arturo", 65456251, 2);
	...
	public void queSePuedanAdministrarDistintosTiposDePolizas() {
		Integer numeroDePoliza = 1, cantidadDePolidasEsperadas = 4;
		CompaniaDeSeguro libra = new CompaniaDeSeguro("Libra");
	
		libra.agregarPoliza(new PolizaDeAuto(1, new Persona("Camila", 41987345, 24), 2000000.0, 5000.0));
		...
		assertEquals( cantidadDePolidasEsperadas, libra.obtenerLaCantidadDePolizasEmitidas());
	}
